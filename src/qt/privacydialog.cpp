                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNXTON ought to be enough for anybody." - Bill Gates, 2017
    ui->zNXTONpayAmount->setValidator( new QDoubleValidator(0.0, 100000000.0, 20, this) );
    ...
    ui->labelzNXTONSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNXTON->setEnabled(false);
        ui->pushButtonMintzNXTON->setToolTip(tr("zNXTON is currently disabled due to maintenance."));

        ui->pushButtonSpendzNXTON->setEnabled(false);
        ui->pushButtonSpendzNXTON->setToolTip(tr("zNXTON is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNXTONpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNXTON_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNXTON is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNXTON...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNXTON in ") +
...
void PrivacyDialog::on_pushButtonSpendzNXTON_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNXTON is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNXTON
        sendzNXTON();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNXTON
    sendzNXTON();
}

void PrivacyDialog::on_pushButtonZNXTONControl_clicked()
{
    ZNXTONControlDialog* zNXTONControl = new ZNXTONControlDialog(this);
    zNXTONControl->setModel(walletModel);
    zNXTONControl->exec();
}

void PrivacyDialog::setZNXTONControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNXTONSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNXTON()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid NXTON Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNXTONpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNXTON is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NXTON </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNXTON</b>";
    ...
    // use mints from zNXTON selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNXTONControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNXTONControlDialog::GetSelectedMints();
    }
    ...
    // Clear znxton selector in case it was used
    ZNXTONControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNXTON Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " NXTON, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNXTON Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNXTON </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNXTON "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNXTON "));
    ui->labelzNXTONAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNXTONSyncStatus->setVisible(fShow);
}
