...
#include "ui_znxtoncontroldialog.h"
...
std::list<std::string> ZNXTONControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNXTONControlDialog::listMints;

ZNXTONControlDialog::ZNXTONControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNXTONControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNXTONControlDialog::~ZNXTONControlDialog()
{
    delete ui;
}

void ZNXTONControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNXTONControlDialog::updateList()
{
...
void ZNXTONControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNXTONControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNXTON_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNXTONControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNXTONControlDialog::GetSelectedMints()
{
...
void ZNXTONControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
